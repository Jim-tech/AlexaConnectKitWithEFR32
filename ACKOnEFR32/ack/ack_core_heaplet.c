...
static void* SetSize_(HeapletTag_t tag, size_t newSize, unsigned flags)
{
    ...
    if ((0 == (flags & HEAPLET_SETSIZE_FLAG_IGNORE_HIGH_WATER_MARK))
        && (sg_pBufferUsedEnd - sg_buffer > 0)
        && ((uint32_t)(sg_pBufferUsedEnd - sg_buffer) > g_maximumHeapletConsumed))
    {
...
{
    HeapletBlockHeader_t* p = FindBlock_(tag, NULL);
