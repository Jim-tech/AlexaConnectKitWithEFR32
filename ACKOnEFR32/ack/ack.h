...
// Typedef for additional information if current state is not connected to Alexa.
typedef struct _ACKNotConnectedToAlexaInfo_t
{
    acp_wifi_disconnection_reason DisconnectionReason;

    // These fields are all a single bit to save space.
    unsigned IsDistressBeaconing: 1;
}
ACKNotConnectedToAlexaInfo_t;
...
typedef struct _ACKStatus_t
{
    ...
    union
    {
        ...
        ACKNotConnectedToAlexaInfo_t NotConnectedToAlexa;
    }
    SubStateInfo;
...
// properties to an outbound event being constructed. If this is for a compound device (ACK_COMPOUND_DEVICE
// is set) then you also need to set which endpoint a property belongs to.
...
// The changedPropertiesBits value tells Alexa which properties actually changed. The otherPropertiesBits
...
// Parameters associated with directives.
typedef struct _ACKDirectiveParams_t
{
...
ACKDirectiveParams_t;

// An application can provide its own handling for capabilities. To do this, it puts
//
//  #define ACK_OTHER_CAPABILITY
//
// in its ack_user_config.h. This requires the application to implement ACKUser_OnDirective, to which directives in
// unhandled namespaces are delivered. It also makes ACK_AddProperty available to the application to add properties
// in artibtary namespaces to event messages (see ACKUser_PropertyTable above).
#ifdef ACK_OTHER_CAPABILITY

void ACKUser_OnDirective(const ACKDirectiveParams_t* pDirectiveParams);
...
#endif // ACK_OTHER_CAPABILITY

#ifdef ACK_COMPOUND_DEVICE
// Returns an endpoint id of a directive if present.
// Must be called within the ACKUser_On* handler.
