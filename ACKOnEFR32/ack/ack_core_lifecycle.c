...
static void SetLifecycleStateWithSubStateCheck(ACKLifecycleState_t newState, bool subStateChanged);
...
// This buffer is read as 16 bit values and on a Cortex-M0 memory accesses must be aligned.
// sg_otaChunkBuffer is intialized so that it is always aligned on 2 bytes. ie. has an even address
// There needs to be an extra byte in this buffer, that way it can be shifted by 1 and still have
// the correct size.
uint8_t sg_otaChunkBuffer0[ACK_HOST_FIRMWARE_UPDATE_CHUNK_SIZE + 1];
uint8_t* sg_otaChunkBuffer = NULL;
...
static void OtaFailed(const char* pWhy);
#define OTA_FAILED(why) OtaFailed(why);
#else
static void OtaFailed(void);
#define OTA_FAILED(why) OtaFailed();
#endif
...
void ACK_Initialize(void)
{
    ...
    ACK_DEBUG_PRINT_C(
        "ACK Device SDK Implementation Core (re-)started; firmware version 0x%"PRIx32"%08"PRIx32,
        (uint32_t)(firmwareVersion >> 32),
...
#ifdef ACK_HOST_FIRMWARE_UPDATE
    // Initialize this buffer so that it is always aligned on 2 bytes. ie. has an even address
    // The original buffer has one extra byte to accommodate for this.
    sg_otaChunkBuffer = ((uintptr_t)sg_otaChunkBuffer0 & 1) ? (sg_otaChunkBuffer0 + 1) : sg_otaChunkBuffer0;
#endif // def ACK_HOST_FIRMWARE_UPDATE
...
void ACK_Process()
{
    ...
    // Standard re-entry check. We also have a separate check to catch applications that call ACK_Process
    // multiple times simultaneously (such from a timer service routine, or periodically from an RTOS task, etc.).
    ACK_REENTRY_CHECK_(ACK_REENTRY_GENERAL_CASE | ACK_SIMULTANEOUS_ACKPROCESS);
    ACK_REENTRY_ACQUIRE_(ACK_SIMULTANEOUS_ACKPROCESS);
...
cleanup0:
    // Automatically release short hold locks
    UNLOCK_POWER_SAVING();

    ACK_REENTRY_RELEASE_(ACK_SIMULTANEOUS_ACKPROCESS);
}
...
{
    static uint32_t userGuidedSetupCheckLastTick = 0;
    static uint8_t userGuidedSetupCheckAttempts = 0;
    ACKError_t err;
    ...
    if ((ACK_LIFECYCLE_UNAVAILABLE == ACK_LifecycleState)
        ...
        || deviceInUse)
    {
        userGuidedSetupCheckAttempts = 0;
        return false;
    }

    // If initiating UGS failed the first time, then check the status periodically
    if (userGuidedSetupCheckAttempts > 0)
    {
        uint32_t tick = ACKPlatform_TickCount();
        if (tick - userGuidedSetupCheckLastTick >= ACK_USER_GUIDED_SETUP_FAILURE_CHECK_INTERVAL_MS)
        {
            ACK_DEBUG_PRINT_I("User-guided setup delayed check, attempt %d",
                ACK_USER_GUIDED_SETUP_FAILURE_CHECK_COUNT - userGuidedSetupCheckAttempts + 1);

            userGuidedSetupCheckAttempts--;
            userGuidedSetupCheckLastTick = tick;

            // Force a GetStatus check to determine if UGS has initiated
            sg_connectivityLastTick = 0;
            CheckAndUpdateConnectivityState();

            ACK_DEBUG_PRINT_D("User-guided setup delayed check, current lifecycle %u",
                ACK_LifecycleState);

            if ((ACK_LIFECYCLE_IN_SETUP_MODE == ACK_LifecycleState)
                && ACK_LifecycleSubStateInfo.InSetupMode.IsUserGuidedSetupActive)
            {
                userGuidedSetupCheckAttempts = 0;
                ACK_DEBUG_PRINT_I("User-guided setup successfully initiated.");
            }
            else if (userGuidedSetupCheckAttempts == 0)
            {
                ACK_DEBUG_PRINT_E("Unable to initiate user-guided setup.");
            }
        }

        // Return true if the status needs to be checked again
        return userGuidedSetupCheckAttempts > 0;
    }
    ...
    err = ACKModule_UserGuidedSetupInitiate();

    if (ACK_ERR_IO == err)
    {
        userGuidedSetupCheckAttempts = ACK_USER_GUIDED_SETUP_FAILURE_CHECK_COUNT;
        userGuidedSetupCheckLastTick = ACKPlatform_TickCount();
        ACK_DEBUG_PRINT_W("User-guided setup didn't immediately initiate, "
            "starting delayed periodic check.");

        // UGS initiation didn't succeed, but the status needs to be checked later
        return true;
    }
...
bool SatisfyPendingFactoryReset(bool deviceInUse)
{
    ...
    else
    {
        ...
        err = ACKModule_FactoryResetInitiate();
...
bool SatisfySendHostConfig(void)
{
    ...
    err = ACKModule_HostConfigWrite(version);
...
bool SatisfyOtaAvailable(void)
{
    ...
    if (0 == g_otaState.RetrieveRemaining)
    {
        ...
        LONG_UNLOCK_POWER_SAVING();
        g_otaState.ApplicationPending = true;
        VALIDATION_SEND_MARKER(ACK_MARKER_OTA_COMPLETE, "success", NULL);
        return true;
    }
...
static uint32_t RetrieveNextOtaChunk(void)
{
    ...
    if (0 == offset)
    {
        ...
        ACK_STATIC_ASSERT(ACK_HOST_FIRMWARE_UPDATE_CHUNK_SIZE >= sizeof(ACKHostFirmwareUpdateImageHeader_t));
        readSize = sizeof(ACKHostFirmwareUpdateImageHeader_t);
    }
    else
    {
        readSize = g_otaState.RetrieveRemaining;
        if (readSize > ACK_HOST_FIRMWARE_UPDATE_CHUNK_SIZE)
        {
            readSize = ACK_HOST_FIRMWARE_UPDATE_CHUNK_SIZE;
        }
    }
    ...
    err = ACKModule_OtaDownload(offset, readSize, sg_otaChunkBuffer, &retrievedSize);
    if (ACK_NO_ERROR == err)
    {
...
#ifdef ACK_VALIDATION
    VALIDATION_SEND_MARKER(ACK_MARKER_OTA_COMPLETE, pWhy, NULL);
#endif
}
        ...
        && (ACK_LIFECYCLE_FACTORY_RESET_IN_PROGRESS != ACK_LifecycleState))
    {
            ...
            || ((tick - sg_connectivityLastTick) >= ACK_CONNECTIVITY_STATE_SAMPLE_INTERVAL_MS))
        {
            ...
            case ACK_IN_SETUP_MODE:
                ...
                #ifdef ACK_NOTIFY_SUBSTATE_CHANGES
                if ((ACK_LifecycleSubStateInfo.InSetupMode.SetupStage
                    != ackStatus.SubStateInfo.InSetupMode.SetupStage)
                    || (ACK_LifecycleSubStateInfo.InSetupMode.IsUserGuidedSetupActive
                    != ackStatus.SubStateInfo.InSetupMode.IsUserGuidedSetupActive)
                    || (ACK_LifecycleSubStateInfo.InSetupMode.IsBarcodeScanSetupActive
                    != ackStatus.SubStateInfo.InSetupMode.IsBarcodeScanSetupActive)
                    || (ACK_LifecycleSubStateInfo.InSetupMode.IsZeroTouchSetupActive
                    != ackStatus.SubStateInfo.InSetupMode.IsZeroTouchSetupActive))
                {
            ...
            case ACK_NOT_CONNECTED_TO_ALEXA:
                ...
                #ifdef ACK_NOTIFY_SUBSTATE_CHANGES
                if ((ACK_LifecycleSubStateInfo.NotConnectedToAlexa.DisconnectionReason
                    != ackStatus.SubStateInfo.NotConnectedToAlexa.DisconnectionReason)
                    || (ACK_LifecycleSubStateInfo.NotConnectedToAlexa.IsDistressBeaconing
                    != ackStatus.SubStateInfo.NotConnectedToAlexa.IsDistressBeaconing))
                {
                    subStateChanged = true;
                }
                #endif

                ACK_LifecycleSubStateInfo.NotConnectedToAlexa = ackStatus.SubStateInfo.NotConnectedToAlexa;
                break;
            ...
            SetLifecycleStateWithSubStateCheck(newState, subStateChanged);
        }
    }
}
...
static void SetLifecycleState(ACKLifecycleState_t newState)
{
    SetLifecycleStateWithSubStateCheck(newState, false);
}

static void SetLifecycleStateWithSubStateCheck(ACKLifecycleState_t newState, bool subStateChanged)
{
    if ((newState != ACK_LifecycleState) || subStateChanged)
    {
    ...
    bool force)
{
    ...
    // Acquire long hold lock for the full duration of the OTA
    LONG_LOCK_POWER_SAVING();
}
